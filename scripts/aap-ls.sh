...
ensure_description() {
  ...
  local has_goals=0
  if list_goal_dirs "$node" | read -r -d '' _; then
    has_goals=1
  fi
...
traverse() {
  ...
  if [[ -z "$first_not_achieved" && "$status" == "not-achieved" ]]; then
    first_not_achieved="$node"
  fi
  if (( seen_not_achieved )) && [[ "$status" == "achieved" ]]; then
...
[[ -n "$first_not_achieved" ]] || die "No not-achieved plan node found under $(rel_to_planroot "$OBJECTIVE_TREE")."

desired_current="$first_not_achieved"
